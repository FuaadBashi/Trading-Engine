...
TEST(BitstampSnapshot, ParsesEveryOrderFromBothSides) {
    const auto result = te::parseBitstampSnapshot(kSample, btcUsd());
...
TEST(BitstampSnapshot, AcceptsEmptySides) {
    const auto result = te::parseBitstampSnapshot(
...
TEST(BitstampSnapshot, RejectsEmptyInputAsMalformedJson) {
    const auto result = te::parseBitstampSnapshot("", btcUsd());

    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::malformed_json);
}
...
TEST(BitstampSnapshot, RejectsGarbageInputWithoutAcceptingIt) {
    for (const char* garbage : {"this is not json", R"({"microtimestamp":)", "12345", "[1,2,3]",
                                R"({"hello":"world"})"}) {
        const auto result = te::parseBitstampSnapshot(garbage, btcUsd());
        EXPECT_FALSE(result.hasValue()) << "silently accepted: " << garbage;
    }
}

TEST(BitstampSnapshot, RejectsMissingMicrotimestamp) {
    const auto result = te::parseBitstampSnapshot(R"({"bids":[],"asks":[]})", btcUsd());

    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::missing_microtimestamp);
}
...
TEST(BitstampSnapshot, RejectsMissingBids) {
    ...
    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::missing_bids);
}
...
TEST(BitstampSnapshot, RejectsMissingAsks) {
    ...
    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::missing_asks);
}
...
TEST(BitstampSnapshot, RejectsRowWithTooFewFields) {
    ...
    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::wrong_field_count);
}
...
TEST(BitstampSnapshot, RejectsNonNumericPrice) {
    ...
    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::invalid_price);
}
...
TEST(BitstampSnapshot, RejectsNonNumericQuantity) {
    ...
    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::invalid_quantity);
}
...
TEST(BitstampSnapshot, RejectsNonNumericOrderId) {
    ...
    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::invalid_order_id);
}
...
TEST(BitstampSnapshot, RejectsDuplicateOrderIdOnTheSameSide) {
    ...
    ASSERT_FALSE(result.hasValue());
    EXPECT_EQ(*result.errorIf(), te::SnapshotError::duplicate_order_id);
}
...
TEST(BitstampSnapshot, ParsesTheCommittedFixtureFile) {
    ...
    const auto result = te::parseBitstampSnapshot(buffer.str(), btcUsd());
...
TEST(BitstampSnapshot, ParsesTheRealCaptureSnapshot) {
    ...
    std::size_t buys = 0;
    std::size_t sells = 0;
    for (const te::SnapshotOrder& order : snapshot.orders) {
